- [一、线程上下文切换](#一线程上下文切换)
  ...
  - [1. 2 案例说明](#1-2-案例说明)
  - [1. 3 切换原因](#1-3-切换原因)
  - [1. 4 减少上下文切换](#1-4-减少上下文切换)
- [二、多线程](#二多线程)
  - [2. 1 进程与线程](#2-1-进程与线程)
  - [2. 2 并发与并行](#2-2-并发与并行)
  - [2. 3 同步与异步](#2-3-同步与异步)
  - [2. 4 Thread创建线程](#2-4-thread创建线程)
  - [2. 5 Runnable创建线程](#2-5-runnable创建线程)
  - [2. 6 匿名内部类创建线程](#2-6-匿名内部类创建线程)
  - [2. 7 守护线程](#2-7-守护线程)
- [二、线程安全](#二线程安全)
  - [3. 1 Java内存模型（JMM）](#3-1-java内存模型jmm)
  - [3. 2 线程安全](#3-2-线程安全)
  - [3. 2 线程同步](#3-2-线程同步)
- [四、生命周期](#四生命周期)
  - [4. 1 概述](#4-1-概述)
  - [4. 2 新建状态(NEW)](#4-2-新建状态new)
  - [4. 3 运行状态(RUNNABLE)](#4-3-运行状态runnable)
  - [4. 4 阻塞状态(BLOCKED)](#4-4-阻塞状态blocked)
  - [4. 5 等待状态(WAITING)](#4-5-等待状态waiting)
  - [4. 6 超时等待状态(TIMED_WAITING)](#4-6-超时等待状态timed_waiting)
  - [4. 7 终止状态（TERMINATED）](#4-7-终止状态terminated)
- [五、等待唤醒机制](#五等待唤醒机制)
  - [5. 1 线程间通信](#5-1-线程间通信)
  - [5. 2 等待唤醒机制](#5-2-等待唤醒机制)
  - [5. 3 生产者与消费者模式](#5-3-生产者与消费者模式)
- [六、线程池](#六线程池)
  - [6. 1 概述](#6-1-概述)
  - [6. 2 线程池原理](#6-2-线程池原理)
  - [6. 3 线程池的使用](#6-3-线程池的使用)
- [参考资料](#参考资料)
...
| Blocked(阻塞)            | 当一个线程试图获取一个对象锁，而该对象锁被其他的线程持有，则该线程进入Blocked状 态；当该线程持有锁时，该线程将变成Runnable状态。 |
| Waiting(无限 等待)       | 一个线程在等待另一个线程执行一个（唤醒）动作时，该线程进入Waiting状态。进入这个状态后是不能自动唤醒的，必须等待另一个线程调用notify或者`notifyAll`方法才能够唤醒。 |
