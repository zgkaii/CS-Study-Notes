...
`sleep()`与锁无关，线程睡眠到时间就会自动苏醒，并返回到Runnable（可运行）中的就绪状态。
...
就拿生产茶叶/消费茶叶来说等待唤醒机制如何有效利用资源：

~~~java
茶叶铺线程生产茶叶，消费者线程消费茶叶。当茶叶没有时（茶叶状态为false），消费者线程等待，茶叶铺线程生产茶叶（即茶叶状态为true），并通知吃货线程（解除吃货的等待状态），因为已经有茶叶了，那么茶叶铺线程进入等待状态。接下来，吃货线程能否进一步执行则取决于锁的获取情况。如果吃货获取到锁，那么就执行吃茶叶动作，茶叶吃完（茶叶状态为false），并通知茶叶铺线程（解除茶叶铺的等待状态）,吃货线程进入等待。茶叶铺线程能否进一步执行则取决于锁的获取情况。
~~~
...
public class BaoZi {
     String  pier ;
     String  xianer ;
     boolean  flag = false ;//茶叶资源 是否存在  茶叶资源状态
}
~~~
    ...
    public void run() {
        while(true){
            synchronized (bz){
                ...
                System.out.println("吃货正在吃"+bz.pier+bz.xianer+"茶叶");
                bz.flag = false;
                bz.notify();
            }
        }
    ...
    public void run() {
        ...
        while(true){
            ...
            synchronized (bz){
                ...
                bz.flag=true;
                System.out.println("茶叶造好了："+bz.pier+bz.xianer);
...
public class Demo {
    public static void main(String[] args) {
        ...
        BaoZiPu bzp = new BaoZiPu("茶叶铺",bz);
...
[一篇文章彻底弄懂生产者--消费者问题](https://juejin.im/post/6844903602469732360)
