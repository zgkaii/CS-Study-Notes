...
`java.lang.Thread.State`枚举出了六种线程状态，可以调用线程Thread中的`getState()`方法**获取当前线程的状态**。

| 线程状态                 | 导致状态发生条件                                             |
| :----------------------- | ------------------------------------------------------------ |
| NEW(新建)                | 尚未启动的线程状态，即线程创建，**还未调用start方法**        |
| Runnable(可 运行)        | **就绪状态**（调用start，等待调度）+**正在运行。**           |
| Blocked(锁阻塞)          | **等待监视器锁**时，陷入阻塞状态。                           |
| Waiting(无限 等待)       | 等待状态的线程正在**等待**另一线程执行特定的操作（如`notify`/`notifyAll`） |
| Timed Waiting(计时 等待) | 具有**指定等待时间**的等待状态。                             |
| `Teminated`( 终止)       | 线程完成执行，**终止状态**                                   |

<img src="https://img-blog.csdnimg.cn/20200929152050615.png" style="zoom:80%;" />
